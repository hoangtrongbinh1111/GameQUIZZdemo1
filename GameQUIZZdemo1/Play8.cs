...
namespace GameQUIZZdemo1
{
     public partial class Play8 : Form
     {
          ...
          public Play8(int surNum1 = 60, string nameClient = "", int pointClient = 0,bool sound=true)
          {
               InitializeComponent();
               surNum = surNum1;
               ...
               Text = "Màn 8 - Player: " + name;//ghi tên vòng với tên người chơi
               soundMode = sound;
          }
          ...
          private void label2_MouseMove(object sender, MouseEventArgs e)//chữ di chuyển
          {
               if (e.Button == System.Windows.Forms.MouseButtons.Left)
               {
                    label2.Left = e.X + label2.Left - MouseDownLocation.X;
                    label2.Top = e.Y + label2.Top - MouseDownLocation.Y;
               }
               if(label2.Location.Y>=390&&label2.Location.Y<=400&& label2.Location.X>=120&& label2.Location.X<=160)
               {
                    timer1.Start();
               }
          }
          ...
          private void bRes_Click(object sender, EventArgs e)//restart
          {
               ...
               {
                    this.Hide();
                    Play8 play = new Play8(surNum, name, point, soundMode);
                    play.ShowDialog();
                    this.Close();
               }
          }
          ...
          private void bNextControl_Click(object sender, EventArgs e)
          {
               ...
               if (dlr == DialogResult.Yes)//đồng ý thì sẽ chuyển sang câu tiếp theo
               {
                    ...
                    else
                    {
                         ...
                         updateDB();
                         this.Hide();
                         Play9 play = new Play9(surNum, name, point, soundMode);
                         play.ShowDialog();
                         this.Close();
                    }
               }
          ...
          private void bInstruct_Click(object sender, EventArgs e)
          {
               ...
               if (countInstruct == 1)
               {
                    ...
                    else
                    {
                         ...
                         MessageBox.Show("Kéo câu hỏi xuống để làm cầu", "Đáp án", MessageBoxButtons.OK);
                    }
               }
          ...
          private void Sound(int mode)
          {
               if (soundMode == true)
               {
                    ...
                    {
                         SoundPlayer s = new SoundPlayer(GameQUIZZdemo1.Properties.Resources.wrong_4);
                         s.Play();
                    }
                    else if (mode == 2)//âm thanh đúng
                    {
                         SoundPlayer s = new SoundPlayer(GameQUIZZdemo1.Properties.Resources.true_Answer);
                         s.Play();
                    }
                    else if (mode == 3)//âm thanh click
                    {
                         SoundPlayer s = new SoundPlayer(GameQUIZZdemo1.Properties.Resources.click4);
                         s.Play();
                    }
